...
class Search extends SearchCore
{
    public static function find(
        ...
        if ($ajax) {
            ...
            Db::getInstance()->execute('SET @xd = 0', false);
            ...
            $sql = 'SELECT DISTINCT p.id_product, pl.name pname, cl.name cname,
					...
					ORDER BY CASE WHEN (pl.`name` LIKE "%Royal%") and @xd<2 THEN @xd AND @xd = @xd+1 ELSE 4 END ASC,
					position DESC LIMIT 10';
        ...
        if (!Tools::getIsset('order')) {
            $sql.= ' ORDER BY CASE WHEN (pl.`name` LIKE "%Royal%") and @xd<2 THEN @xd AND @xd = @xd+1 ELSE 4 END ASC,
                        ' . ($order_by ? $alias . $order_by : '') . ($order_way ? ' ' .
                    $order_way : '');
