...
if (!defined('_PS_VERSION_'))
	exit;
class ms_manufacturerslider extends Module
{
    private $_html;
    public function __construct()
	{
		$this->name = 'ms_manufacturerslider';
		$this->tab = 'front_office_features';
		$this->version = '2.0.0';
		$this->author = 'Millenium Studio - Sebastian Koziołek';
		...
		$this->displayName = $this->l('Slider logo');
        $this->description = $this->l('Slider logo producentów na stornie głównej');
$this->refs = 'https://milleniumstudio.pl/';
	...
	public function install()
	{
        ...
        return parent::install() 
               ...
               && $this->registerHook('msManufacturer');
    }
    ...
    {
        $manufacturers=$this->getManufactures("CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, name asc",false,true);
				...
				'input' => array(
                    ...
                    array(
                		...
                		'options' => array(
                         ...
                         'desc' => $this->l('Put {hook h=\'msManufacturer\'} to .tpl files where you want to display the manufacturer logos'),
                    ),                  
                ),
                'submit' => array(
					'title' => $this->l('Save'),
     ...
     * Hooks 
     */
     public function getManufactures($orderby = "CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, name asc", $limit = false,$all=false)
     {
     ...
     {        
        ...
        {
            case 'name_desc':
                $order = "CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, name desc";
                break;
            case 'manu_asc':
                $order = "CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, id_manufacturer asc";
                break;
            case 'manu_desc':
                $order = "CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, id_manufacturer desc";
                break;
            default:
                $order = "CASE WHEN (name LIKE '%Royal%') THEN 0 ELSE 1 END, name asc";
                break;
        }
		...
		{		
            if(file_exists(_PS_MANU_IMG_DIR_.$manufacturer['id_manufacturer'].'.jpg'))
                $manufacturer['image'] = $this->context->link->getManufacturerImageLink((int)$manufacturer['id_manufacturer'], '');
            else
        ...
        if (Tools::getValue('controller')=='index') {
            ...
            return $this->display(__FILE__, 'manufacturers-site.tpl');
        }
        
     }
     public function hookmsManufacturer($params)
     {
     ...
     public function hookDisplayBackOfficeHeader()
    {
        ...
        if(Tools::getValue('controller') == 'AdminModules' && Tools::getValue('configure') == 'ms_manufacturerslider'){
            $this->smarty->assign(array(
     ...
     public function hookDisplayHeader()
     {
        ...
        // {
            $this->context->controller->addCSS($this->_path.'views/css/ms_manufacturerslider.css','all');
            $this->context->controller->addJS($this->_path.'views/js/owl.carousel.js'); 
            $this->context->controller->addJS($this->_path.'views/js/ms_manufacturerslider.js');            
