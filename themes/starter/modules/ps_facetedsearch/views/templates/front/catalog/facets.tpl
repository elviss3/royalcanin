      ...
      <div class="list-group list-group-flush">
        {block name='facets_clearall_button'}
          {if $activeFilters|count}
            <div class="clear-all-wrapper card-body d-none d-lg-block">
          ...
          {/if}
            <div class="block_modal_buttons d-flex d-lg-none justify-content-between">
              <button type="button" class="btn btn-block btn-primary p-1 m-2 width-fit-content" data-dismiss="modal" aria-label="Close">
                {l s='Show' d='Shop.Istheme'}
              </button>
              {if $activeFilters|count}
              <div class="mobile_clear_filters p-2">
                <button data-search-url="{$clear_all_link}" class="display-toggle__link js-search-filters-clear-all">
...
{literal}
<script>
  $('.search-filters__collapse').each(function(){
    var $ul = $(this),
        $lis = $ul.find('.py-1:gt(4)'),
        isExpanded = $ul.hasClass('expanded');   
    $lis[isExpanded ? 'show' : 'hide']();
    
    if($lis.length > 0){
        $ul
            .append($('<div class="expand"><span>' + (isExpanded ? '- mniej' : '+ więcej') + '</span></div>')
            .click(function(event){
                var isExpanded = $ul.hasClass('expanded');
                event.preventDefault();
                $(this).text(isExpanded ? '+ więcej' : '- mniej');
